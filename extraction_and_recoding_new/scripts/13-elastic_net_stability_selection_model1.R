# =========================================================
# Purpose: Stability selection elastic net (60% split)
# =========================================================
...
# 9. & 10. Tune Alpha via Stability Score & Run Stability Selection
message("\n=== Tuning Elastic Net Alpha via Stability Score ===")
# Define a grid of alpha values as requested
alpha_grid <- seq(0.1, 0.9, by = 0.1)
best_alpha <- NULL
max_stability_score <- -Inf
best_out_stability <- NULL

for (a in alpha_grid) {
  message("Testing alpha = ", a, " ...")
  set.seed(seed_value)
  # Run stability selection separately for each alpha
  temp_out <- VariableSelection(
    xdata          = X,
    ydata          = y,
    family         = "binomial",
    penalty.factor = pf,
  ...
  # Compute the maximum stability score for this model
  current_max_score <- max(temp_out$S, na.rm = TRUE)
  message("  -> Max Stability Score: ", round(current_max_score, 4))
  # Select the alpha that maximises the stability score
  if (current_max_score > max_stability_score) {
    max_stability_score <- current_max_score
    best_alpha <- a
    best_out_stability <- temp_out
  }
}

message("\n=== Tuning Complete ===")
message("Best alpha selected: ", best_alpha, " (Score: ", round(max_stability_score, 4), ")")

# Assign the best model to our main output variable
out_stability <- best_out_stability
# 11. Calibration
pdf(file.path(output_dir, "model1_calibration_plot.pdf"), width = 10, height = 7)
...
# 12. Selection Proportions Plot
selprop <- SelectionProportions(out_stability)
pdf(file.path(output_dir, "model1_selection_proportions.pdf"), width = 14, height = 7)
par(mar = c(10, 5, 1, 1))
...
# 14. Save Outputs
saveRDS(out_stability, file.path(output_dir, "model1_stability_object.rds"))
write.csv(
  data.frame(
  ...
  file.path(output_dir, "model1_stability_summary.csv"),
  row.names = FALSE
)
write.csv(all_selprop_df, file.path(output_dir, "model1_all_selection_proportions.csv"), row.names = FALSE)
write.csv(stable_vars_final, file.path(output_dir, "model1_stable_variables.csv"), row.names = FALSE)
write.csv(filter(stable_vars_final, var_type == "exposure"), 
          file.path(output_dir, "model1_stable_exposures.csv"), row.names = FALSE)
message("=== Selection Stage Complete ===")