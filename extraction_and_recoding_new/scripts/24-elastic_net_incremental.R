# ============================================================
# SCRIPT 2: ELASTIC NET INCREMENTAL AUC PLOT
# Includes:
# - base model AUC
...
p_cum_auc_exposure <- ggplot(
  ...
  labs(
    title = "Elastic Net: Incremental AUC of Stable Exposures",
    x = "Model step",
    y = "AUC"
  ) +
...
cat("Saved: ../outputs/elastic_net_incremental_auc_exposure.png\n")
...
p_cum_auc_all <- ggplot(
  ...
  labs(
    title = "Elastic Net: Incremental AUC of Stable Variables",
    x = "Model step",
    y = "AUC",
...
cat("Saved: ../outputs/elastic_net_incremental_auc_all.png\n")
...
cat("\n=== Final summary ===\n")
print(data.frame(
  model = "Elastic Net-selected refit logistic model",
  auc = auc_val,
  auc_low = auc_ci[1],
  auc_high = auc_ci[3]
))
...
cat("\nSaved files:\n")
cat("- ../outputs/elastic_net_incremental_auc_summary_exposure.csv\n")
cat("- ../outputs/elastic_net_incremental_auc_summary_all.csv\n")
cat("- ../outputs/elastic_net_incremental_auc_exposure.png\n")
cat("- ../outputs/elastic_net_incremental_auc_all.png\n")