# ============================================================
# SCRIPT: TOTAL + DIRECT LASSO LOGISTIC REGRESSION REFIT
#         + REPEATED 50% SUBSAMPLING
#         + FOREST PLOTS BASED ON SUBSAMPLING RESULTS
#
# PURPOSE
# - Refit standard logistic regression after LASSO stability selection
...
#
# TOTAL EFFECT
# - outputs/total_lasso_logistic_summary.csv
# - outputs/total_lasso_subsample_summary.csv
# - outputs/total_lasso_forest_all.pdf          [optional]
#
# DIRECT EFFECT
# - outputs/direct_lasso_logistic_summary.csv
# - outputs/direct_lasso_subsample_summary.csv
# - outputs/direct_lasso_forest_exposure.pdf
#
# NOTES
...
stable_file <- "../outputs/summary/model1_lasso_stable_variables.csv"
...
make_subsample_forest_plot <- function(
    ...
    forest_xlim = NULL,
    forest_breaks = NULL,
    forest_pad = 0.05,              # padding added before rounding x-limits
    break_by = 0.10,                # default x-axis step
    pdf_width = 22,
    row_height = 0.48,
    min_pdf_height = 10
) {
  ...
  # Auto forest x-limits:
  # add padding, then round outward to 1 decimal place
  ...
  # Safer pretty breaks using a fixed interval
  compute_forest_breaks <- function(xlim, by = 0.10) {
    seq(xlim[1], xlim[2], by = by)
  }
  
  # Estimate text panel layout in "character units"
  # This avoids hard-coding left/right panel positions.
  compute_left_panel_layout <- function(var_labels, level_labels) {
    ...
    # gap between variable and level text columns
    gap_chars <- -6
    
    # x positions in character units
    var_x   <- var_chars
    level_x <- var_chars + gap_chars + level_chars
    
    # x-limits for left text panel
  ...
  compute_right_panel_layout <- function(label_txt_vec) {
    ...
    # start text a little away from panel edge
    text_x <- 1.5
    
    # x-limits in character units
  ...
  compute_panel_widths <- function(left_weight, forest_xlim, right_weight, break_by = 0.10) {
    ...
    # forest panel weight based on number of break intervals, with a floor
  ...
  # ============================================================
  # Cap plotted CI to forest panel limits only
  # ============================================================
  ...
  var_text_df <- forest_data %>%
    ...
    summarise(
      y_var = max(y_id),   # align variable with first displayed level
      .groups = "drop"
    ) %>%
    mutate(
  ...
  # ============================================================
  # Left panel: variable names + levels
  # ============================================================
...
run_lasso_pipeline <- function(
  ...
  # ------------------------------------------------------------
  # Selected variables from stable selection
  # ------------------------------------------------------------
  ...
  # Keep only variables present in refit dataset
  ...
  # ------------------------------------------------------------
  # Conventional logistic regression summary
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  # Repeated 50% subsampling for stability analysis
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  # Summarise terms across subsamples
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  # Add variable / level information for plotting
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  logistic_file  <- paste0(analysis_name, "_lasso_logistic_summary.csv")
  subsample_file <- paste0(analysis_name, "_lasso_subsample_summary.csv")
  ...
  # ------------------------------------------------------------
  # Forest plots from subsampling summaries
  # ------------------------------------------------------------
  if (make_all_plot) {
    forest_all_file <- paste0(analysis_name, "_lasso_forest_all.pdf")
    
    make_subsample_forest_plot(
      res_df     = model_tbl,
      vars       = full_model_vars,
      title      = paste("Forest Plot of", str_to_title(analysis_name), "LASSO-Selected Variables"),
  ...
  if (make_exposure_plot) {
    forest_exp_file <- paste0(analysis_name, "_lasso_forest_exposure.pdf")
    
    make_subsample_forest_plot(
      res_df     = model_tbl,
      vars       = plot_vars_exposure,
      title      = paste("Lasso:", str_to_title(analysis_name), "Exposure Variables Based on Repeated Subsampling"),
...
cat("============================================================\n")
cat("Saved files:\n")
cat("- ../outputs/total_lasso_logistic_summary.csv\n")
cat("- ../outputs/total_lasso_subsample_summary.csv\n")
if (make_total_plot) {
  cat("- ../outputs/total_lasso_forest_all.pdf\n")
}
cat("- ../outputs/direct_lasso_logistic_summary.csv\n")
cat("- ../outputs/direct_lasso_subsample_summary.csv\n")
cat("- ../outputs/direct_lasso_forest_exposure.pdf\n")