...
N_TRIALS = 50
CV_FOLDS = 5
TOP_N    = 20   # number of SHAP features fo fallback if the maximum number of features form lasso and e-net fails
class Logger:
    def __init__(self):
...
## making the new N_top features selected based on lasso and e-net
def _read_n_stable(csv_path, col="n_stable_total"):
    try:
        return int(pd.read_csv(csv_path)[col].iloc[0])
    except Exception:
        return 0
...
if TOP_N == 0:
    TOP_N = 20
    print(f"[WARN] Could not read stability summary CSVs — using fallback TOP_N = {TOP_N}\n")
else:
    print(f">>> TOP_N set dynamically from stability selection outputs:")
    print(f"    Lasso        n_stable_total = {_lasso_n}")
    print(f"    Elastic Net  n_stable_total = {_elastic_n}")
    print(f"    TOP_N = max({_lasso_n}, {_elastic_n}) = {TOP_N}\n")


# ─── 1. LOAD DATA ────────────────────────────────────────────────────────────
print(">>> Loading data...")
result_train = pyreadr.read_r("../outputs/ukb_selection_60_imputed.rds")
result_val   = pyreadr.read_r("../outputs/ukb_refit_20_imputed.rds")
result_test  = pyreadr.read_r("../outputs/ukb_test_20_imputed.rds")
df_train = result_train[None]
df_val   = result_val[None]
...
for _df in [df_train, df_val, df_test]:
    _df["outcome_binary"] = np.where(
        _df["outcome"].astype(str).str.strip() == "No", 0, 1
    )
...
print(f"    Train — Cases: {n_cases} | Controls: {n_controls} | "
      f"Prevalence: {prevalence:.2%}")
print(f"    Val   — Cases: {int(df_val['outcome_binary'].sum())} | "
      f"Controls: {int((df_val['outcome_binary']==0).sum())}")
print(f"    Test  — Cases: {int(df_test['outcome_binary'].sum())} | "
      f"Controls: {int((df_test['outcome_binary']==0).sum())}\n")
...
always_drop = [c for c in always_drop if c in df_train.columns]
X_train = df_train.drop(columns=always_drop)
y_train = df_train["outcome_binary"]
X_val   = df_val.drop(columns=[c for c in always_drop if c in df_val.columns])
y_val   = df_val["outcome_binary"]
X_test  = df_test.drop(columns=[c for c in always_drop if c in df_test.columns])
y_test  = df_test["outcome_binary"]

# Make val/test columns identical to train columns, in the same order
X_val = X_val.reindex(columns=X_train.columns)
X_test = X_test.reindex(columns=X_train.columns)

assert list(X_train.columns) == list(X_val.columns), "X_val columns do not match X_train"
assert list(X_train.columns) == list(X_test.columns), "X_test columns do not match X_train"

# Encode categoricals — fit categories on train, apply consistently to val and test
...
if cat_cols:
    ...
    for col in cat_cols:
        ...
        X_val[col]   = X_val[col].astype("category").cat.set_categories(train_cats)
        X_test[col]  = X_test[col].astype("category").cat.set_categories(train_cats)
    X_train[cat_cols] = X_train[cat_cols].apply(lambda col: col.cat.codes).replace(-1, np.nan)
    X_val[cat_cols]   = X_val[cat_cols].apply(lambda col: col.cat.codes).replace(-1, np.nan)
    X_test[cat_cols]  = X_test[cat_cols].apply(lambda col: col.cat.codes).replace(-1, np.nan)
N_FEATURES_TOTAL = X_train.shape[1]
print(f"    Feature matrix: {X_train.shape[0]} train | {X_val.shape[0]} val | "
      f"{X_test.shape[0]} test | {N_FEATURES_TOTAL} features\n")
      
# ─── 4. YOUDEN'S J THRESHOLD ─────────────────────────────────────────────────
...
# ─── 5. HYPERPARAMETER TUNING — runs ONCE on all features ────────────────────
def tune_hyperparameters(X_tr, y_tr, X_vl, y_vl):
    ...
    def objective(trial):
        ...
        return average_precision_score(y_vl, m.predict_proba(X_vl)[:, 1])
    print(f">>> Hyperparameter tuning on ALL {N_FEATURES_TOTAL} features")
    print(f"    {N_TRIALS} trials | metric = AUC-PR | validation set for early stopping")
    study = optuna.create_study(direction="maximize",
                                sampler=optuna.samplers.TPESampler(seed=42))
    study.optimize(objective, n_trials=N_TRIALS, show_progress_bar=False)
    print(f"    Best val AUC-PR : {study.best_value:.4f}")
    print(f"    Best params     : {study.best_params}\n")
    return study.best_params, study
best_params, cv_study = tune_hyperparameters(X_train, y_train, X_val, y_val)

# ─── 6. TRAIN & EVALUATE HELPER ──────────────────────────────────────────────
def train_model(X_tr, X_te, y_tr, y_te, label, params, X_vl=None, y_vl=None):
    """Train on X_tr, use X_vl for early stopping, evaluate on X_te."""
    if X_vl is None:
        X_vl, y_vl = X_te, y_te
    neg, pos = (y_tr == 0).sum(), (y_tr == 1).sum()
    spw = neg / pos
...
# ─── 6b. INCREMENTAL SHAP AUC HELPER ─────────────────────────────────────────
def incremental_shap_auc(shap_ranked_features, X_tr, y_tr, X_te, y_te, params,
    ...
    Trains XGBoost at each step, uses X_vl for early stopping, records AUC-ROC on X_te.
...
# ─── 7. MODEL A — ALL FEATURES (trained once) ────────────────────────────────
...
# ─── 8b. INCREMENTAL AUC — one feature added per step ────────────────────────
...
# ─── 9. MODEL B — SHAP-SELECTED FEATURES (same tuned params, retrained) ─────
...
results_B = train_model(
    ...
    params=best_params,
    X_vl=X_val[selected_features], y_vl=y_val,
)
# ─── 10. COMPARISON TABLE ────────────────────────────────────────────────────
...
with PdfPages(pdf_path) as pdf:
    ...
    add_text_page(pdf, "1. Dataset Summary", [
        f"Total rows:        {X_train.shape[0] + X_val.shape[0] + X_test.shape[0]}",
        f"Train (60%):       {X_train.shape[0]}",
        f"Val   (20%):       {X_val.shape[0]}",
        f"Test  (20%):       {X_test.shape[0]}",
        f"Cases (train):     {n_cases}",
        f"Controls (train):  {n_controls}",
        f"Prevalence:        {prevalence:.2%}",
