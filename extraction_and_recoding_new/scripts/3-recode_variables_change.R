...
for (k in 1:ncol(mydata)) {
  ...
  if (!is.na(tmp_coding_id)) {
    ...
    x_chr <- as.character(mydata[, k])
    
    recoding <- coding$RecodedMeaning
    names(recoding) <- coding$OriginalValue
    
    out <- x_chr
    hit <- !is.na(x_chr) & x_chr %in% names(recoding)
    out[hit] <- unname(recoding[x_chr[hit]])
   ...
   # }
  #}
  valtype <- tolower(as.character(choices[tmp_coding_name, "ValueType"]))
  
  if (grepl("integer|continuous", valtype)) {
    tmp <- as.character(recoded_data[, k])
    
    # allows: -7, 12, 3.14 (and NA)
    num_pattern <- "^-?\\d+(\\.\\d+)?$"
    
    # convert ONLY if every non-missing value is numeric text
    if (all(is.na(tmp) | grepl(num_pattern, tmp))) {
      recoded_data[, k] <- as.numeric(tmp)
    } else {
      recoded_data[, k] <- tmp  # keep as character (preserves your strings)
    }
  }
...
write.xlsx(choices, "../parameters/parameters_changed.xlsx")

# Saving extracted dataset
saveRDS(mydata, "../outputs/ukb_recoded_changed.rds")
